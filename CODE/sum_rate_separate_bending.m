function [sum_r_temp] = sum_rate_separate_bending(psi,y_UPA,z_UPA,Nh,Nv,K,L,phi,theta,alpha,lambda,P,Ant_type,ka,sector_index)
d=lambda/2;

y_FAA=(Nh-1)*d/2/psi*sin(psi*y_UPA/((Nh-1)*d/2));  % y-position mapping
x_FAA=-(Nh-1)*d/2/psi+(Nh-1)*d/2/psi*cos(psi*y_UPA/((Nh-1)*d/2))+d*(Nh-1)*(1+sqrt(3)/3)/pi;  % x-position mapping
z_FAA=z_UPA;
    ...
    for l=1:L
        for ny=1:Nh
            switch sector_index
                ...
                case 2
                    psi_ny=2*psi/(Nh-1)*(ny-1)-psi+2*pi/3;
                otherwise
                    psi_ny=2*psi/(Nh-1)*(ny-1)-psi+4*pi/3;
            end
            for nz=1:Nv
                if Ant_type==0
