function [sum_r_temp] = sum_rate_semi_joint_bending(psi1,psi2,psi3,y_UPA,z_UPA,Nh,Nv,K,L,phi1,phi2,phi3,theta1,theta2,theta3,alpha1,alpha2,alpha3,lambda,P,Ant_type,ka)
d=lambda/2;

y_FAA1=(Nh-1)*d/2/psi1*sin(psi1*y_UPA/((Nh-1)*d/2));  % y-position mapping
x_FAA1=-(Nh-1)*d/2/psi1+(Nh-1)*d/2/psi1*cos(psi1*y_UPA/((Nh-1)*d/2))+d*(Nh-1)*(1+sqrt(3)/3)/pi;  % x-position mapping
z_FAA1=z_UPA;

y_FAA2_ini=(Nh-1)*d/2/psi2*sin(psi2*y_UPA/((Nh-1)*d/2));  % y-position mapping
x_FAA2_ini=-(Nh-1)*d/2/psi2+(Nh-1)*d/2/psi2*cos(psi2*y_UPA/((Nh-1)*d/2))+d*(Nh-1)*(1+sqrt(3)/3)/pi;  % x-position mapping
z_FAA2=z_UPA;

y_FAA3_ini=(Nh-1)*d/2/psi3*sin(psi3*y_UPA/((Nh-1)*d/2));  % y-position mapping
x_FAA3_ini=-(Nh-1)*d/2/psi3+(Nh-1)*d/2/psi3*cos(psi3*y_UPA/((Nh-1)*d/2))+d*(Nh-1)*(1+sqrt(3)/3)/pi;  % x-position mapping
z_FAA3=z_UPA;

Rot2=[cos(2*pi/3) -sin(2*pi/3);sin(2*pi/3) cos(2*pi/3)];
p_FAA2=Rot2*[x_FAA2_ini;y_FAA2_ini];
x_FAA2=p_FAA2(1,:);
y_FAA2=p_FAA2(2,:);
Rot3=[cos(4*pi/3) -sin(4*pi/3);sin(4*pi/3) cos(4*pi/3)];
p_FAA3=Rot3*[x_FAA3_ini;y_FAA3_ini];
x_FAA3=p_FAA3(1,:);
y_FAA3=p_FAA3(2,:);
...
for k=1:K*3
    for l=1:L
        for ny=1:Nh 
            psi_ny1=-psi1+2*psi1/(Nh-1)*(ny-1);
            psi_ny2=-psi2+2*psi2/(Nh-1)*(ny-1)+2*pi/3;
            psi_ny3=-psi3+2*psi3/(Nh-1)*(ny-1)+4*pi/3;
            for nz=1:Nv
                if Ant_type==0
                ...
                if Ant_type==1
                    %              	r_cyl(ny,nz)=(x_FAA1(ny)*cos(phi1(l,k))*sin(theta1(l,k))+y_FAA1(ny)*sin(phi1(l,k))*sin(theta1(l,k))+z_FAA1(nz)*cos(theta1(l,k)));
                    %                 er_cyl(ny,nz)=exp(-1i*2*pi/lambda*r_cyl(ny,nz))*CosineAnt(theta1(l,k),phi1(l,k)-psi_ny1,ka);
                    %             	r_cy2(ny,nz)=(x_FAA2(ny)*cos(phi2(l,k))*sin(theta2(l,k))+y_FAA2(ny)*sin(phi2(l,k))*sin(theta2(l,k))+z_FAA2(nz)*cos(theta2(l,k)));
                    %                 er_cy2(ny,nz)=exp(-1i*2*pi/lambda*r_cy2(ny,nz))*CosineAnt(theta2(l,k),phi2(l,k)-psi_ny2,ka);
                    %                 r_cy3(ny,nz)=(x_FAA3(ny)*cos(phi3(l,k))*sin(theta3(l,k))+y_FAA3(ny)*sin(phi3(l,k))*sin(theta3(l,k))+z_FAA3(nz)*cos(theta3(l,k)));
                    %                 er_cy3(ny,nz)=exp(-1i*2*pi/lambda*r_cy3(ny,nz))*CosineAnt(theta3(l,k),phi3(l,k)-psi_ny3,ka);
