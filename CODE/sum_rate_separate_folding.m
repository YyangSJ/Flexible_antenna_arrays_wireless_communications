function [sum_r_temp] = sum_rate_separate_folding(psi,x_UPA,y_UPA,z_UPA,Nh,Nv,K,L,phi,theta,alpha,lambda,P,Ant_type,ka,sector_index)
d=lambda/2;

Rot1_local=[cos(psi) -sin(psi);sin(psi) cos(psi)];
p_FAA=Rot1_local*[x_UPA;y_UPA];
if psi>0
    x_FAA=-abs(p_FAA(1,:))+d*(Nh-1)*(1+sqrt(3)/3)/pi;
else
    x_FAA=abs(p_FAA(1,:))+d*(Nh-1)*(1+sqrt(3)/3)/pi;
end
y_FAA=p_FAA(2,:);
