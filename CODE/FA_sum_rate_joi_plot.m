...
for reali=1:realization
    ...
    for p=1:length(PP)
        ...
        fun = @(x)sum_rate_joint_bending(x.psi1,x.psi2,x.psi3,y_UPA,z_UPA,Nh,Nv,K,L,phi1,phi2,phi3,theta1,theta2,theta3,alpha1,alpha2,alpha3,lambda,P,0,0); %Create Function Handles
        ...
        fun = @(x)sum_rate_joint_bending(x.psi1,x.psi2,x.psi3,y_UPA,z_UPA,Nh,Nv,K,L,phi1,phi2,phi3,theta1,theta2,theta3,alpha1,alpha2,alpha3,lambda,P,1,1); %Create Function Handles
        results = bayesopt(fun,vars,'InitialX',T_psi_ini,'MaxObjectiveEvaluations',100,'ParallelMethod','clipped-model-prediction','PlotFcn',[]);
        Sum_rate_FAA_bending_ka1(reali,p)=-results.MinObjective;
        
        
        fun = @(x)sum_rate_joint_bending(x.psi1,x.psi2,x.psi3,y_UPA,z_UPA,Nh,Nv,K,L,phi1,phi2,phi3,theta1,theta2,theta3,alpha1,alpha2,alpha3,lambda,P,1,2); %Create Function Handles
        results = bayesopt(fun,vars,'InitialX',T_psi_ini,'MaxObjectiveEvaluations',100,'ParallelMethod','clipped-model-prediction','PlotFcn',[]);
        Sum_rate_FAA_bending_ka2(reali,p)=-results.MinObjective;
        
        Sum_rate_FIX_omni(reali,p)=-sum_rate_joint_bending(1e-6,1e-6,1e-6,y_UPA,z_UPA,Nh,Nv,K,L,phi1,phi2,phi3,theta1,theta2,theta3,alpha1,alpha2,alpha3,lambda,P,0,0)
        Sum_rate_FIX_ka1(reali,p)=-sum_rate_joint_bending(1e-6,1e-6,1e-6,y_UPA,z_UPA,Nh,Nv,K,L,phi1,phi2,phi3,theta1,theta2,theta3,alpha1,alpha2,alpha3,lambda,P,1,1)
        Sum_rate_FIX_ka2(reali,p)=-sum_rate_joint_bending(1e-6,1e-6,1e-6,y_UPA,z_UPA,Nh,Nv,K,L,phi1,phi2,phi3,theta1,theta2,theta3,alpha1,alpha2,alpha3,lambda,P,1,2)
        ...
        fun =  @(x)sum_rate_joint_rotating(x.psi1,x.psi2,x.psi3,x_UPA,y_UPA,z_UPA,Nh,Nv,K,L,phi1,phi2,phi3,theta1,theta2,theta3,alpha1,alpha2,alpha3,lambda,P,0,0);
        results = bayesopt(fun,vars,'InitialX',T_psi_ini,'MaxObjectiveEvaluations',100,'ParallelMethod','clipped-model-prediction','PlotFcn',[]);
        Sum_rate_FAA_rotating_omni(reali,p)=-results.MinObjective;
        
        fun =  @(x)sum_rate_joint_rotating(x.psi1,x.psi2,x.psi3,x_UPA,y_UPA,z_UPA,Nh,Nv,K,L,phi1,phi2,phi3,theta1,theta2,theta3,alpha1,alpha2,alpha3,lambda,P,1,1);
        results = bayesopt(fun,vars,'InitialX',T_psi_ini,'MaxObjectiveEvaluations',100,'ParallelMethod','clipped-model-prediction','PlotFcn',[]);
        Sum_rate_FAA_rotating_ka1(reali,p)=-results.MinObjective;
        fun =  @(x)sum_rate_joint_rotating(x.psi1,x.psi2,x.psi3,x_UPA,y_UPA,z_UPA,Nh,Nv,K,L,phi1,phi2,phi3,theta1,theta2,theta3,alpha1,alpha2,alpha3,lambda,P,1,2);
        results = bayesopt(fun,vars,'InitialX',T_psi_ini,'MaxObjectiveEvaluations',100,'ParallelMethod','clipped-model-prediction','PlotFcn',[]);
        ...
        vars = [
            ...
            optimizableVariable('psi2',[-pi/6,pi/6],'Type','real'),
            optimizableVariable('psi3',[-pi/6,pi/6],'Type','real') ];
        fun =  @(x)sum_rate_joint_folding(x.psi1,x.psi2,x.psi3,x_UPA,y_UPA,z_UPA,Nh,Nv,K,L,phi1,phi2,phi3,theta1,theta2,theta3,alpha1,alpha2,alpha3,lambda,P,0,0);
        results = bayesopt(fun,vars,'InitialX',T_psi_ini,'MaxObjectiveEvaluations',100,'ParallelMethod','clipped-model-prediction','PlotFcn',[]);
        Sum_rate_FAA_folding_omni(reali,p)=-results.MinObjective;
        
        fun =  @(x)sum_rate_joint_folding(x.psi1,x.psi2,x.psi3,x_UPA,y_UPA,z_UPA,Nh,Nv,K,L,phi1,phi2,phi3,theta1,theta2,theta3,alpha1,alpha2,alpha3,lambda,P,1,1);
        results = bayesopt(fun,vars,'InitialX',T_psi_ini,'MaxObjectiveEvaluations',100,'ParallelMethod','clipped-model-prediction','PlotFcn',[]);
        Sum_rate_FAA_folding_ka1(reali,p)=-results.MinObjective;
        fun =  @(x)sum_rate_joint_folding(x.psi1,x.psi2,x.psi3,x_UPA,y_UPA,z_UPA,Nh,Nv,K,L,phi1,phi2,phi3,theta1,theta2,theta3,alpha1,alpha2,alpha3,lambda,P,1,2);
        results = bayesopt(fun,vars,'InitialX',T_psi_ini,'MaxObjectiveEvaluations',100,'ParallelMethod','clipped-model-prediction','PlotFcn',[]);
